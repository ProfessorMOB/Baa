...
wchar_t *process_string(BaaPreprocessor *pp_state, const wchar_t *source_string, wchar_t **error_message)
{
    ...
    while (*line_start != L'\0' && success)
    {
        ...
        {
            wchar_t *directive_start = current_line + 1;
            pp_state->current_column_number = 2;
            while (iswspace(*directive_start))
            {
                directive_start++;
                pp_state->current_column_number++;
            ...
            const wchar_t *include_directive = L"تضمين";
            size_t include_directive_len = wcslen(include_directive);
            // ... (rest of directive definitions copied from process_file) ...
            const wchar_t *define_directive = L"تعريف";
            size_t define_directive_len = wcslen(define_directive);
            const wchar_t *undef_directive = L"الغاء_تعريف";
            size_t undef_directive_len = wcslen(undef_directive);
            ...
            bool is_conditional_directive = false;
            PpSourceLocation directive_loc = get_current_original_location(pp_state);

            // --- Process Conditional Directives FIRST --- (Copied from process_file)
             if (wcsncmp(directive_start, if_directive, if_directive_len) == 0 &&
                (directive_start[if_directive_len] == L'\0' || iswspace(directive_start[if_directive_len])))
            {
                is_conditional_directive = true;
                ...
                if (*expr_start == L'\0') {
                    *error_message = format_preprocessor_error_at_location(&directive_loc, L"تنسيق #إذا غير صالح: التعبير مفقود."); success = false;
                } else {
                    bool expr_value = false;
                    // NOTE: Passing NULL for abs_path as it's not relevant for string input evaluation context
                    if (!evaluate_preprocessor_expression(pp_state, expr_start, &expr_value, error_message, NULL)) {
                        if (!*error_message) *error_message = format_preprocessor_error_at_location(&directive_loc, L"خطأ في تقييم تعبير #إذا.");
                        success = false;
                    } else {
                        if (!push_conditional(pp_state, expr_value)) {
                            *error_message = format_preprocessor_error_at_location(&directive_loc, L"فشل في دفع الحالة الشرطية لـ #إذا (نفاد الذاكرة؟)."); success = false;
                        }
                    }
                }
            }
                 ...
                 if (pp_state->conditional_stack_count == 0) {
                     *error_message = format_preprocessor_error_at_location(&directive_loc, L"#وإلا_إذا بدون #إذا/#إذا_عرف/#إذا_لم_يعرف مطابق."); success = false;
                     ...
                     if (pp_state->conditional_branch_taken_stack[top_index]) {
                         ...
                         while (iswspace(*expr_start)) expr_start++;
                         if (*expr_start == L'\0') {
                             *error_message = format_preprocessor_error_at_location(&directive_loc, L"تنسيق #وإلا_إذا غير صالح: التعبير مفقود."); success = false;
                         } else {
                             bool condition_met = false;
                             // NOTE: Passing NULL for abs_path
                             if (!evaluate_preprocessor_expression(pp_state, expr_start, &condition_met, error_message, NULL)) {
                                 if (!*error_message) *error_message = format_preprocessor_error_at_location(&directive_loc, L"خطأ في تقييم تعبير #وإلا_إذا.");
                                 success = false;
                             } else {
                                 if (condition_met) {
                                     pp_state->conditional_stack[top_index] = true;
            ...
            // --- End Conditional Directives ---

            // --- Process other directives ONLY if not skipping AND not conditional ---
            if (!is_conditional_directive && !pp_state->skipping_lines)
            {
                if (wcsncmp(directive_start, include_directive, include_directive_len) == 0 &&
                    (directive_start[include_directive_len] == L'\0' || iswspace(directive_start[include_directive_len])))
                {
                    // #include is generally not meaningful/possible from a string source without a base path.
                    *error_message = format_preprocessor_error_at_location(&directive_loc, L"لا يمكن استخدام #تضمين مباشرة من مصدر سلسلة.");
                    success = false;
                }
                else if (wcsncmp(directive_start, define_directive, define_directive_len) == 0 &&
                         (directive_start[define_directive_len] == L'\0' || iswspace(directive_start[define_directive_len])))
                {
                    // --- #define logic (copied from process_file) ---
                    wchar_t *name_start = directive_start + define_directive_len;
                    pp_state->current_column_number += define_directive_len;
                    ...
                    if (name_start == name_end) {
                        *error_message = format_preprocessor_error_at_location(&directive_loc, L"تنسيق #تعريف غير صالح: اسم الماكرو مفقود."); success = false;
                    } else {
                        size_t name_len = name_end - name_start;
                        wchar_t *macro_name = wcsndup_internal(name_start, name_len);
                        if (!macro_name) {
                            *error_message = format_preprocessor_error_at_location(&directive_loc, L"فشل في تخصيص ذاكرة لاسم الماكرو في #تعريف."); success = false;
                        } else {
                    ...
                    // --- End #define logic ---
                         ...
                         (directive_start[undef_directive_len] == L'\0' || iswspace(directive_start[undef_directive_len])))
                {
                    // --- #undef logic (copied from process_file) ---
                     wchar_t *name_start = directive_start + undef_directive_len; pp_state->current_column_number += undef_directive_len;
                     while (iswspace(*name_start)) { name_start++; pp_state->current_column_number++; }
                     wchar_t *name_end = name_start; while (*name_end != L'\0' && !iswspace(*name_end)) name_end++;
                     if (name_start == name_end) {
                         *error_message = format_preprocessor_error_at_location(&directive_loc, L"تنسيق #الغاء_تعريف غير صالح: اسم الماكرو مفقود."); success = false;
                     } else {
                         size_t name_len = name_end - name_start; wchar_t *macro_name = wcsndup_internal(name_start, name_len);
        ...
        {
            // --- Macro substitution logic (copied from process_file) ---
            DynamicWcharBuffer substituted_line_buffer;
            if (!init_dynamic_buffer(&substituted_line_buffer, line_len + 128)) {
                PpSourceLocation error_loc = get_current_original_location(pp_state);
                *error_message = format_preprocessor_error_at_location(&error_loc, L"فشل في تخصيص الذاكرة لمخزن السطر المؤقت للاستبدال."); success = false;
            } else {
                const wchar_t *line_ptr = current_line;
                while (*line_ptr != L'\0' && success) {
                    size_t current_char_col = pp_state->current_column_number;
                    if (iswalpha(*line_ptr) || *line_ptr == L'_') {
                        const wchar_t *id_start = line_ptr; size_t id_start_col = current_char_col;
                        while (iswalnum(*line_ptr) || *line_ptr == L'_') { line_ptr++; pp_state->current_column_number++; }
                        size_t id_len = line_ptr - id_start; wchar_t *identifier = wcsndup_internal(id_start, id_len);
                        if (!identifier) { PpSourceLocation error_loc = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&error_loc, L"فشل في تخصيص ذاكرة للمعرف للاستبدال."); success = false; break; }
                        if (wcscmp(identifier, L"__الملف__") == 0) {
                            wchar_t quoted_file_path[MAX_PATH_LEN + 3]; PpSourceLocation current_original_loc = get_current_original_location(pp_state); const char* original_file_path = current_original_loc.file_path ? current_original_loc.file_path : "unknown_file";
                            wchar_t w_current_physical_path[MAX_PATH_LEN]; mbstowcs(w_current_physical_path, original_file_path, MAX_PATH_LEN);
                            wchar_t escaped_path[MAX_PATH_LEN * 2]; wchar_t *ep = escaped_path; for (const wchar_t *p = w_current_physical_path; *p; ++p) { if (*p == L'\\') { *ep++ = L'\\'; } *ep++ = *p; } *ep = L'\0';
                            swprintf(quoted_file_path, sizeof(quoted_file_path)/sizeof(wchar_t), L"\"%ls\"", escaped_path);
                            if (!append_to_dynamic_buffer(&substituted_line_buffer, quoted_file_path)) { success = false; } free(identifier);
                        } else if (wcscmp(identifier, L"__السطر__") == 0) {
                            wchar_t line_str[20]; swprintf(line_str, sizeof(line_str)/sizeof(wchar_t), L"\"%zu\"", pp_state->current_line_number);
                            if (!append_to_dynamic_buffer(&substituted_line_buffer, line_str)) { success = false; } free(identifier);
                        } else {
                            const BaaMacro *macro = find_macro(pp_state, identifier);
                            if (macro && !is_macro_expanding(pp_state, macro)) {
                                PpSourceLocation invocation_loc = {.file_path = pp_state->current_file_path, .line = pp_state->current_line_number, .column = id_start_col};
                                if (!push_location(pp_state, &invocation_loc)) { *error_message = format_preprocessor_error_at_location(&invocation_loc, L"فشل في دفع موقع استدعاء الماكرو (نفاد الذاكرة؟)."); success = false; free(identifier); break; }
                                if (!push_macro_expansion(pp_state, macro)) { *error_message = format_preprocessor_error_at_location(&invocation_loc, L"فشل في دفع الماكرو '%ls' إلى مكدس التوسيع (نفاد الذاكرة؟).", macro->name); pop_location(pp_state); success = false; free(identifier); break; }
                                bool expansion_success = true;
                                if (macro->is_function_like) {
                                    const wchar_t *invocation_ptr = line_ptr; size_t col_before_args = pp_state->current_column_number; while (iswspace(*invocation_ptr)) { invocation_ptr++; pp_state->current_column_number++; }
                                    if (*invocation_ptr == L'(') {
                                        invocation_ptr++; pp_state->current_column_number++; size_t actual_arg_count = 0; wchar_t **arguments = parse_macro_arguments(pp_state, &invocation_ptr, macro->param_count, &actual_arg_count, error_message);
                                        if (!arguments) { expansion_success = false; }
                                        else {
                                            if (actual_arg_count != macro->param_count) { PpSourceLocation current_loc = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&current_loc, L"عدد وسيطات غير صحيح للماكرو '%ls' (متوقع %zu، تم الحصول على %zu).", macro->name, macro->param_count, actual_arg_count); expansion_success = false; }
                                            else {
                                                DynamicWcharBuffer expansion_result_buffer;
                                                if (!init_dynamic_buffer(&expansion_result_buffer, 128)) { PpSourceLocation current_loc = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&current_loc, L"فشل في تهيئة المخزن المؤقت لنتيجة توسيع الماكرو '%ls'.", macro->name); expansion_success = false; }
                                                else {
                                                    if (!substitute_macro_body(pp_state, &expansion_result_buffer, macro, arguments, actual_arg_count, error_message)) { expansion_success = false; }
                                                    else { if (!append_to_dynamic_buffer(&substituted_line_buffer, expansion_result_buffer.buffer)) { expansion_success = false; } }
                                                    free_dynamic_buffer(&expansion_result_buffer);
                                                }
                                            }
                                            for (size_t i = 0; i < actual_arg_count; ++i) free(arguments[i]); free(arguments);
                                        }
                                        line_ptr = invocation_ptr;
                                    } else { pp_state->current_column_number = col_before_args; if (!append_dynamic_buffer_n(&substituted_line_buffer, id_start, id_len)) expansion_success = false; }
                                } else {
                                    DynamicWcharBuffer expansion_result_buffer;
                                    if (!init_dynamic_buffer(&expansion_result_buffer, 128)) { PpSourceLocation current_loc = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&current_loc, L"فشل في تهيئة المخزن المؤقت لنتيجة توسيع الماكرو '%ls'.", macro->name); expansion_success = false; }
                                    else {
                                        if (!substitute_macro_body(pp_state, &expansion_result_buffer, macro, NULL, 0, error_message)) { expansion_success = false; }
                                        else { if (!append_to_dynamic_buffer(&substituted_line_buffer, expansion_result_buffer.buffer)) { expansion_success = false; } }
                                        free_dynamic_buffer(&expansion_result_buffer);
                                    }
                                }
                                pop_macro_expansion(pp_state); pop_location(pp_state);
                                if (!expansion_success) { success = false; }
                            } else if (macro) { PpSourceLocation current_loc = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&current_loc, L"تم اكتشاف استدعاء ذاتي للماكرو '%ls'.", macro->name); success = false; }
                            else { if (!append_dynamic_buffer_n(&substituted_line_buffer, id_start, id_len)) success = false; }
                            free(identifier);
                        }
                    } else { if (!append_dynamic_buffer_n(&substituted_line_buffer, line_ptr, 1)) success = false; line_ptr++; pp_state->current_column_number++; }
                }
                if (success) { if (!append_to_dynamic_buffer(&output_buffer, substituted_line_buffer.buffer)) success = false; }
                free_dynamic_buffer(&substituted_line_buffer);
            }
            // --- End Macro substitution logic ---

            if (success && !append_to_dynamic_buffer(&output_buffer, L"\n")) success = false;
            if (!success && !*error_message) { PpSourceLocation current_loc = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&current_loc, L"فشل في إلحاق السطر بمخزن الإخراج المؤقت."); }
        }
