...
wchar_t *process_file(BaaPreprocessor *pp_state, const char *file_path, wchar_t **error_message)
{
    *error_message = NULL;
    ...
    while (*line_start != L'\0' && success)
    {
        ...
        {
            fwprintf(stderr, L"DEBUG: Processing directive line %zu: [%ls]\n", pp_state->current_line_number, effective_line_start); // DEBUG PRINT
            // Trim leading whitespace after '#' is already handled by effective_line_start logic
            // Adjust directive_start based on effective_line_start
            wchar_t *directive_start = effective_line_start + 1;
            // pp_state->current_column_number is already past leading whitespace + '#'

            // Define directive keywords and lengths
            // REMOVED Redundant definition: wchar_t *directive_start = current_line + 1;
            // Reset column tracking based on the actual start of the directive keyword
            pp_state->current_column_number = (directive_start - current_line) + 1;

            // No need to skip whitespace again, effective_line_start already handled it.
            // while (iswspace(*directive_start)) { ... } // REMOVED
            ...
            // Define directive keywords and lengths (This block should be inside the '#' check)
        ...
        {
            fwprintf(stderr, L"DEBUG: Processing code line %zu: [%ls]\n", pp_state->current_line_number, current_line); // DEBUG PRINT
                ...
                while (*line_ptr != L'\0' && success)
                {
                    ...
                    if (iswalpha(*line_ptr) || *line_ptr == L'_')
                    {
                        ...
                        else
                        {
                            ...
                            { // This 'else if' pairs with 'if (macro && !is_macro_expanding)'
                                // Recursive expansion
                                PpSourceLocation current_loc = get_current_original_location(pp_state);
                            ...
                            { // This 'else' pairs with 'if (macro && !is_macro_expanding)'
    ...
    *error_message = NULL;
    ...
    while (*line_start != L'\0' && success)
    {
        ...
        if (effective_line_start[0] == L'#')
        {
            ...
            // No need to skip whitespace again
            // while (iswspace(*directive_start)) { ... } // REMOVED
            // REMOVED Stray lines:
            //     directive_start++;
            //     pp_state->current_column_number++;
            ...
            const wchar_t *include_directive = L"تضمين"; // This block should be inside the '#' check
            size_t include_directive_len = wcslen(include_directive);
        ...
        else if (!pp_state->skipping_lines)
        {
            ...
            else
            {
                ...
                while (*line_ptr != L'\0' && success)
                {
                    ...
                    {
                        const wchar_t *id_start = line_ptr;
                        size_t id_start_col = current_char_col;
                        while (iswalnum(*line_ptr) || *line_ptr == L'_')
                        {
                            line_ptr++;
                            pp_state->current_column_number++;
                        ...
                        else
                        {
                            ...
                            if (macro && !is_macro_expanding(pp_state, macro))
                            {
                                ...
                                {
                                    const wchar_t *invocation_ptr = line_ptr;
                                    size_t col_before_args = pp_state->current_column_number;
                                    while (iswspace(*invocation_ptr))
                                    {
                                        invocation_ptr++;
                                        pp_state->current_column_number++;
                                    ...
                                    {
                                        pp_state->current_column_number = col_before_args;
                                        if (!append_dynamic_buffer_n(&substituted_line_buffer, id_start, id_len))
                                            expansion_success = false;
                                    }
                                }
