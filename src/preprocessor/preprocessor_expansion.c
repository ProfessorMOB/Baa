...
bool stringify_argument(BaaPreprocessor *pp_state, DynamicWcharBuffer *output_buffer, const wchar_t *argument, wchar_t **error_message)
{
    ...
    bool success = true;
    if (!append_to_dynamic_buffer(&temp_buffer, L"\"")) { success = false; }
    ...
    while (*ptr != L'\0' && success) {
        ...
        if (!append_to_dynamic_buffer(&temp_buffer, char_to_append)) { success = false; }
        ptr++;
    }

    if (success && !append_to_dynamic_buffer(&temp_buffer, L"\"")) { success = false; }
...
wchar_t **parse_macro_arguments(BaaPreprocessor *pp_state, const wchar_t **invocation_ptr_ref, const BaaMacro *macro, size_t *actual_arg_count, wchar_t **error_message)
{
    ...
    *error_message = NULL;
    ...
    while (*ptr != L'\0') {
        while (iswspace(*ptr)) {
            ptr++;
            if (pp_state) pp_state->current_column_number++;
        }
        ...
        if (*actual_arg_count > 0) {
            ...
            if (macro->is_variadic && *actual_arg_count == named_param_count) {
                ...
                if (named_param_count == 0) { // e.g. FOO(...)
        ...
        if (macro->is_variadic && *actual_arg_count >= named_param_count) {
            ...
            while (*arg_end != L'\0') {
                if (pp_state) pp_state->current_column_number++;
                if (in_string) { if (*arg_end == L'"' && prev_char != L'\\') in_string = false; }
                else if (in_char) { if (*arg_end == L'\'' && prev_char != L'\\') in_char = false; }
                else {
                    if (*arg_end == L'(') paren_level++;
                    else if (*arg_end == L')') {
                        if (paren_level == 0) break; 
                        paren_level--;
                    }
                    else if (*arg_end == L'"') in_string = true;
                    else if (*arg_end == L'\'') in_char = true;
                }
                if (*arg_end == L'\\' && prev_char == L'\\') prev_char = L'\0'; else prev_char = *arg_end;
                arg_end++;
            }
        } else { // Parsing a named argument
            while (*arg_end != L'\0') {
                if (pp_state) pp_state->current_column_number++;
                if (in_string) { if (*arg_end == L'"' && prev_char != L'\\') in_string = false; }
                else if (in_char) { if (*arg_end == L'\'' && prev_char != L'\\') in_char = false; }
                    ...
                    if (*arg_end == L'(') paren_level++;
                    else if (*arg_end == L')') { if (paren_level == 0) break; paren_level--; }
                    else if (*arg_end == L',' && paren_level == 0) break; 
                    else if (*arg_end == L'"') in_string = true;
                    else if (*arg_end == L'\'') in_char = true;
                }
                if (*arg_end == L'\\' && prev_char == L'\\') prev_char = L'\0'; else prev_char = *arg_end;
        ...
        if (paren_level != 0) { PpSourceLocation el = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"أقواس غير متطابقة في وسيطات الماكرو."); goto parse_error; }
        if (in_string || in_char) { PpSourceLocation el = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"علامة اقتباس غير منتهية في وسيطات الماكرو."); goto parse_error; }
        ...
        if (macro->is_variadic && *actual_arg_count == named_param_count && 
            effective_arg_start == effective_arg_end && *ptr == L')') { // Check effective_arg_start == effective_arg_end for empty VA
             arg_len = 0; 
        }

        wchar_t *arg_str = wcsndup_internal(effective_arg_start, arg_len); 
        if (!arg_str) { PpSourceLocation el = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"فشل في تخصيص ذاكرة لوسيطة الماكرو."); goto parse_error; }
        ...
        if (*actual_arg_count >= args_capacity) {
            ...
            if (macro->is_variadic && new_capacity <= named_param_count) new_capacity = named_param_count + 1;
            wchar_t **new_args = realloc(args, new_capacity * sizeof(wchar_t *));
            if (!new_args) { free(arg_str); PpSourceLocation el = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"فشل إعادة تخصيص وسيطات الماكرو."); goto parse_error; }
            args = new_args; args_capacity = new_capacity;
        }
        args[*actual_arg_count] = arg_str;
        (*actual_arg_count)++;

        if (*ptr == L'\0') { PpSourceLocation el = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"قوس إغلاق ')' مفقود في استدعاء الماكرو."); goto parse_error; }
        
        if (macro->is_variadic && *actual_arg_count == named_param_count + 1) { 
             if (*ptr == L')') { ptr++; if (pp_state) pp_state->current_column_number++; break; }
        }
    }
    ...
    if (macro->is_variadic && *actual_arg_count == named_param_count) {
        if (*actual_arg_count >= args_capacity) { 
            ...
            wchar_t **new_args = realloc(args, new_capacity * sizeof(wchar_t *));
            if (!new_args) { PpSourceLocation el = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"فشل تخصيص __VA_ARGS__ فارغ."); goto parse_error; }
            args = new_args; args_capacity = new_capacity;
        }
        args[*actual_arg_count] = _wcsdup(L""); 
        if (!args[*actual_arg_count]) { PpSourceLocation el = get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"فشل نسخ __VA_ARGS__ فارغ."); goto parse_error; }
        (*actual_arg_count)++;
    }
    ...
    if (macro->is_variadic && *actual_arg_count < named_param_count +1) { // Should be param_count + 1 after empty VA_ARGS added
...
parse_error:
    if (args) {
        for (size_t i = 0; i < *actual_arg_count; ++i) free(args[i]);
        free(args);
    }
    *actual_arg_count = 0;
    return NULL;
    ...
    while (*body_ptr != L'\0' && success) {
        if (iswspace(*body_ptr)) {
            if (pending_token_active) {
                if (!append_to_dynamic_buffer(output_buffer, pending_token_buffer.buffer)) { success = false; break; }
                free_dynamic_buffer(&pending_token_buffer);
                if (!init_dynamic_buffer(&pending_token_buffer, 64)) { success = false; break; } 
                pending_token_active = false;
            }
            if (!append_dynamic_buffer_n(output_buffer, body_ptr, 1)) { success = false; break; }
            body_ptr++;
            continue;
        }

        if (*body_ptr == L'#' && *(body_ptr + 1) == L'#') { // Token Pasting
            if (!pending_token_active) { PpSourceLocation el=get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"## في موقع غير صالح بـ '%ls'.", macro->name); success = false; break; }
            body_ptr += 2; 
            ...
            if (macro->is_variadic && wcsncmp(body_ptr, va_args_keyword, va_args_keyword_len) == 0 && !(iswalnum(*(body_ptr + va_args_keyword_len)) || *(body_ptr + va_args_keyword_len) == L'_')) {
                ...
                PpSourceLocation el=get_current_original_location(pp_state); *error_message = format_preprocessor_error_at_location(&el, L"## يجب أن يتبعه معرف أو رقم أو __وسائط_متغيرة__ في '%ls'.", macro->name); success = false; break;
            }
            size_t rhs_len = body_ptr - rhs_start;
            wchar_t *rhs_token_str = wcsndup_internal(rhs_start, rhs_len);
            if (!rhs_token_str) { PpSourceLocation el=get_current_original_location(pp_state);*error_message = format_preprocessor_error_at_location(&el, L"فشل تخصيص RHS لـ ##."); success = false; break;}
            ...
            if (rhs_is_va_args) {
                ...
                for (size_t i = 0; i < macro->param_count; ++i) {
                    if (wcscmp(rhs_token_str, macro->param_names[i]) == 0) {
                        ...
                        rhs_is_param = true; break;
                    }
                }
                if (!rhs_is_param) rhs_value_to_paste = _wcsdup(rhs_token_str);
            }
            free(rhs_token_str);
            if (!rhs_value_to_paste) { PpSourceLocation el=get_current_original_location(pp_state);*error_message = format_preprocessor_error_at_location(&el, L"فشل تخصيص قيمة RHS لـ ##."); success = false; break;}

            if (pending_token_buffer.length == 0) { 
                 if (!append_to_dynamic_buffer(&pending_token_buffer, rhs_value_to_paste)) success = false;
            } else if (rhs_value_to_paste[0] == L'\0') { /* RHS empty, LHS stays */ }
            else { if (!append_to_dynamic_buffer(&pending_token_buffer, rhs_value_to_paste)) success = false; }
            
            free(rhs_value_to_paste);
            if (!success) { PpSourceLocation el=get_current_original_location(pp_state);*error_message = format_preprocessor_error_at_location(&el, L"فشل إلحاق قيمة RHS لـ ##."); break;}
        ...
        if (*body_ptr == L'#') { // Stringify
            const wchar_t *operator_ptr = body_ptr; body_ptr++;
            if (macro->is_variadic && wcsncmp(body_ptr, va_args_keyword, va_args_keyword_len) == 0 && !(iswalnum(*(body_ptr + va_args_keyword_len)) || *(body_ptr + va_args_keyword_len) == L'_')) {
                if (arg_count > macro->param_count) { 
                    if (!stringify_argument(pp_state, output_buffer, arguments[macro->param_count], error_message)) success = false;
                } else { 
                    if (!append_to_dynamic_buffer(output_buffer, L"\"\"")) success = false;
                }
                body_ptr += va_args_keyword_len;
            } else if (iswalpha(*body_ptr) || *body_ptr == L'_') {
                const wchar_t *id_start = body_ptr; while (iswalnum(*body_ptr) || *body_ptr == L'_') body_ptr++;
                size_t id_len = body_ptr - id_start; wchar_t *id_text = wcsndup_internal(id_start, id_len);
                if (!id_text) { PpSourceLocation el=get_current_original_location(pp_state);*error_message = format_preprocessor_error_at_location(&el, L"فشل تخصيص معرف لـ #."); success = false; break;}
                bool param_found = false;
                for (size_t i = 0; i < macro->param_count; ++i) {
                    if (wcscmp(id_text, macro->param_names[i]) == 0) {
                        if (!stringify_argument(pp_state, output_buffer, arguments[i], error_message)) success = false;
                ...
                if (!success) break;
                if (!param_found) { 
                    if (!append_dynamic_buffer_n(output_buffer, operator_ptr, 1)) { success = false; break; }
                    body_ptr = id_start; 
                }
            } else { 
                if (!append_dynamic_buffer_n(output_buffer, operator_ptr, 1)) { success = false; break; }
        ...
        if (iswalpha(*body_ptr) || *body_ptr == L'_') { // Identifier
            const wchar_t *id_start = body_ptr;
            while (iswalnum(*body_ptr) || *body_ptr == L'_') body_ptr++;
            size_t id_len = body_ptr - id_start;
            wchar_t *identifier = wcsndup_internal(id_start, id_len);
            if (!identifier) { PpSourceLocation el=get_current_original_location(pp_state);*error_message = format_preprocessor_error_at_location(&el, L"فشل تخصيص معرف بنص ماكرو '%ls'.", macro->name); success = false; break;}
            ...
            if (macro->is_variadic && wcscmp(identifier, va_args_keyword) == 0) {
                if (arg_count > macro->param_count && arguments[macro->param_count][0] != L'\0') { 
                     if (!append_to_dynamic_buffer(&pending_token_buffer, arguments[macro->param_count])) success = false;
                } 
                substituted = true;
            } else {
                for (size_t i = 0; i < macro->param_count; ++i) {
                    if (wcscmp(identifier, macro->param_names[i]) == 0) {
                        if (arguments[i][0] != L'\0') { 
                            if (!append_to_dynamic_buffer(&pending_token_buffer, arguments[i])) success = false;
                        }
            ...
            if (!substituted) { 
                if (!append_to_dynamic_buffer(&pending_token_buffer, identifier)) success = false;
            }
            free(identifier);
            if (!success) break;
            pending_token_active = true;
        } else { // Single character token
            if (!append_dynamic_buffer_n(&pending_token_buffer, body_ptr, 1)) { success = false; break; }
            body_ptr++;
            pending_token_active = true;
        }
    }
