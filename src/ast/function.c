...
BaaFunction* baa_create_function(const wchar_t* name, size_t name_length) {
    ...
    function->name = baa_wcs_dup_n(name, name_length);
    if (!function->name) {
        baa_free(function);
        return NULL;
    }
...
BaaParameter* baa_create_parameter(const wchar_t* name, size_t name_length, 
    ...
    // Create a copy of the name with proper allocation
    parameter->name = baa_wcs_dup_n(name, name_length);
    if (!parameter->name) {
        baa_free(parameter);
        return NULL;
    }
...
bool baa_add_parameter_to_function(BaaFunction* function, BaaParameter* parameter) {
    ...
    // Check if we need to allocate or resize the parameters array
    if (function->parameter_count >= function->parameter_capacity) {
        ...
        BaaParameter* new_parameters = (BaaParameter*)baa_realloc(
            function->parameters, 
            new_capacity * sizeof(BaaParameter)
        );
    ...
    // The parameter data has been copied, so free the original structure
    // but not its internal data which was transferred
    baa_free(parameter);
