...
void baa_free_stmt(BaaStmt* stmt) {
    ...
    switch (stmt->kind) {
        ...
        case BAA_STMT_BLOCK:
            if (stmt->data) {
                baa_free_block((BaaBlock*)stmt->data);
            }
            break;
            ...
            if (stmt->data) {
                ...
                if (switch_stmt->cases) {
                    for (size_t i = 0; i < switch_stmt->case_count; i++) {
                        BaaCaseStmt* case_stmt = switch_stmt->cases[i];
                        if (case_stmt->value) {
                ...
                if (switch_stmt->default_case) {
                    BaaDefaultStmt* default_stmt = switch_stmt->default_case;
        ...
        case BAA_STMT_CASE:
            if (stmt->data) {
                BaaCaseStmt* case_stmt = (BaaCaseStmt*)stmt->data;
                if (case_stmt->value) {
                    baa_free_expr(case_stmt->value);
                }
                if (case_stmt->body) {
        ...
        case BAA_STMT_DEFAULT:
            if (stmt->data) {
                BaaDefaultStmt* default_stmt = (BaaDefaultStmt*)stmt->data;
                if (default_stmt->body) {
                    baa_free_block(default_stmt->body);
                }
                baa_free(default_stmt);
        ...
        case BAA_STMT_BREAK:
            if (stmt->data) {
                BaaBreakStmt* break_stmt = (BaaBreakStmt*)stmt->data;
                baa_free(break_stmt);
            }
            break;
        ...
        case BAA_STMT_CONTINUE:
            if (stmt->data) {
                BaaContinueStmt* continue_stmt = (BaaContinueStmt*)stmt->data;
                baa_free(continue_stmt);
            }
            break;
    }
