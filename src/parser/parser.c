...
static int get_operator_precedence(const wchar_t* op) {
    if (wcscmp(op, L"=") == 0) return 1;
    if (wcscmp(op, L"||") == 0) return 2;
    if (wcscmp(op, L"&&") == 0) return 3;
    if (wcscmp(op, L"==") == 0 || wcscmp(op, L"!=") == 0) return 4;
    if (wcscmp(op, L"<") == 0 || wcscmp(op, L">") == 0 ||
        wcscmp(op, L"<=") == 0 || wcscmp(op, L">=") == 0) return 5;
    if (wcscmp(op, L"+") == 0 || wcscmp(op, L"-") == 0) return 6;
    if (wcscmp(op, L"*") == 0 || wcscmp(op, L"/") == 0 || wcscmp(op, L"%") == 0) return 7;
    return 0;
}
...
static wchar_t peek(Parser* parser) {
    if (is_eof(parser)) return L'\0';
    return parser->source[parser->position];
}

static wchar_t peek_next(Parser* parser) {
    if (parser->position + 1 >= wcslen(parser->source)) return L'\0';
...
static bool match_keyword(Parser* parser, const wchar_t* keyword) {
    ...
    if (parser->position + len > wcslen(parser->source)) return false;
